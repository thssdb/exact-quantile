...
public class SamplingHeapForStatMerge {
  ...
  public SamplingHeapForStatMerge(int maxMemByte, int maxSeriByte) {
    ...
    this.indexHeap = new IntHeapPriorityQueue(sampleLimit,(x,y)->(Double.compare(score[x],score[y])));
    ...
    this.minV = Long.MAX_VALUE;this.maxV = Long.MIN_VALUE;
  }
...
//    maxV = Math.max(maxV,value);
  ...
  public void merge(List<SamplingHeapForStatMerge> anotherList) {
    for(SamplingHeapForStatMerge another:anotherList)
      merge(another);
  }
...
//    System.out.println("\t\t\t\t\tqueryQ:"+q);
  ...
  public void show(){
    sortSample();System.out.print("\t\t["+indexList.size()+" samples for N="+totN+"]");
    for(int i=0;i<indexList.size();i++)System.out.print("\t("+value[indexList.get(i)]+","+weight[indexList.get(i)]+")");
    System.out.println();
  }
  public void showNum(){
    sortSample();System.out.println("\t\t["+getSampleSize()+" samples for N="+totN+"]");
  }
...
//    return upperBound;
  ...
  public boolean exactResult(){return getSampleSize()==totN;}
  ...
  public SamplingHeapForStatMerge(InputStream inputStream, int maxMemoryByte, int maxSerializeByte) throws IOException {
    this(maxMemoryByte);
