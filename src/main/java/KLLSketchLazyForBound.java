...
public class KLLSketchLazyForBound extends KLLSketchForQuantile {
  public KLLSketchLazyForBound(int maxMemoryByte) {
    N = 0;
...
//    System.out.println();
  ...
  static double[][] PR;
  static final int maxTimesForExact = 25;
  ...
  private double prOutOfBound(double cntPR,int level,int topErr,int bound, int lowLevel,NormalDistribution lowDis,int lowExtreme, int[] relatedCompactNum){
...
//      System.out.println("\t\t\t\t\t\ttopErr="+topErr+"\t\tlimitedBound:"+bound+"\t\tprOfOutOfBound:"+prOutOfBound+"\t\t\tcntPR:"+cntPR);
    ...
    int cntLowerExtreme=lowExtreme;
    for(int i=level-1;i>=lowLevel;i--)cntLowerExtreme+=relatedCompactNum[i]<<i;
    double prSum=0;
    int cntTime = relatedCompactNum[level];
    for(int x=-cntTime;x<=cntTime;x++){
      int cntErr = x*(1<<level);
      if(Math.abs(topErr+cntErr)>bound+cntLowerExtreme){
        prSum+=cntPR*PR[cntTime][x+cntTime];
        continue;
      }
      if(Math.abs(topErr+cntErr)+cntLowerExtreme<=bound){
        continue;
      }
//      if(cntPR>0)
//      System.out.println("\t\t\t\t\t\t\t\t\t\tcntErr:"+cntErr+"\t\tlevel="+level+"\t\tx:"+x+"\t\tcntLevelPr="+PR[cntTime][x+cntTime]+"\t\t\tcntLevelCompNum:"+cntTime+"\tcntLevelV:"+x);

      prSum+=prOutOfBound(cntPR*PR[cntTime][x+cntTime], level-1,topErr+cntErr,bound,lowLevel,lowDis,lowExtreme,relatedCompactNum);
    }
    return prSum;
  }
  private boolean checkBound(int bound, double Pr,int lowLevel,NormalDistribution lowDis,int lowExtreme,int[] relatedCompactNum){
    double prOutOfBound = prOutOfBound(1.0,cntLevel-2,0,bound,lowLevel,lowDis,lowExtreme,relatedCompactNum);
//    System.out.println("\t\tcheckBound.\tbound:"+bound+"\t\t\tprOutOfBound:"+prOutOfBound);
    return prOutOfBound <= 1-Pr;
  }

  static Object2DoubleOpenHashMap<String> queriedBound = new Object2DoubleOpenHashMap<>();
...
//      System.out.println("\t\t\tLOL sameConpactionNumber");
      return queriedBound.getDouble(cntQuery);
    }
...
//    System.out.println();
    ...
    double sig2 = 0.0;
    int lowLevel=0,lowExtreme=0,maxERR=0;
    for(int i=0;i<cntLevel-1;i++)maxERR+=relatedCompactNum[i]<<i;
    for(int i=0;i<cntLevel-1;i++)if(relatedCompactNum[i]>=maxTimesForExact){
