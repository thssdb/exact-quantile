...
//    this.compression = this.clusterNumMemLimit / 11; //  cluster:buffer = 1:10
  ...
  private void addCluster(double v){addCluster(new Cluster(v));}
...
//    System.out.println("\t\t expectedClusterSize:"+expectedClusterSize + " totN:"+totN+" oldClusterNum:"+oldClusterNum);
    for (int i = 0; i < oldClusterNum; i++) {
      if(compression!=this.compression&&oldClusterNum-i+cntClusterNum+(cnt.count>0?1:0)<=maxSeriByte/2/8){ // for serialize.
//        System.out.println("\t\t\t\tpartial compaction!");
        if(cnt.count>0)
          cluster.set(cntClusterNum++,cnt);
//          addCluster(cnt);
        for(;i<oldClusterNum;i++)
          cluster.set(cntClusterNum++,cluster.get(i));
...
//    System.out.println("\t\t add: "+(value));
  ...
  public double getAllMin(){double mn=Double.MAX_VALUE;for(Cluster c:cluster)mn=Math.min(mn,c.min);return mn;}
  public double getAllMax(){double mx=-Double.MAX_VALUE;for(Cluster c:cluster)mx=Math.max(mx,c.max);return mx;}

  private boolean checkPossibleLEQ(Cluster c,long k,double V){ // at least k numbers (exclude min&max) in cluster <= V
    // c.min <= V < c.max
    long k2 = c.count-2-k;
//    boolean flag1 = c.min+c.max + c.min*k + V*k2 <= c.avg*c.count;
    boolean flag1 = c.min/c.count+c.max/c.count + (1.0*k/c.count*c.min) + (1.0*k2/c.count*V) <= c.avg;
//    boolean flag2 = c.min+c.max + (k*V) + (k2*c.max) >= c.avg*c.count;
    boolean flag2 = c.min/c.count+c.max/c.count + (1.0*k/c.count*V) + (1.0*k2/c.count*c.max) >= c.avg;
//    return flag1&&flag2;
    return flag1;
  }
  private boolean checkPossibleGEQ(Cluster c,long k,double V){ // at least k numbers (exclude min&max) in cluster >= V
    // c.min < V <= c.max
    long k2 = c.count-2-k;
    boolean flag1 = c.min/c.count+c.max/c.count + (1.0*k/c.count*V) + (1.0*k2/c.count*c.min) <= c.avg;
    boolean flag2 = c.min/c.count+c.max/c.count + (1.0*k/c.count*c.max) + (1.0*k2/c.count*V) >= c.avg;
//    return flag1&&flag2;
    return flag2;
  }
...
//    System.out.println("\t\tcompactBeforeSeri OVER\t");
  ...
  public void show(){
    ...
    for(int i=0;i<cluster.size();i++)System.out.print("("+ df.format(cluster.get(i).avg)+","+cluster.get(i).count
        +" "+(cluster.get(i).min)+"..."+(cluster.get(i).max)+")"+"\t\t");
//    for(int i=0;i<cluster.size();i++)System.out.print("("+ df.format(cluster.get(i).avg)+","+cluster.get(i).count
//        +" "+df.format(cluster.get(i).min)+"..."+df.format(cluster.get(i).max)+")"+"\t\t");
//    for(int i=0;i<cluster.size();i++)System.out.print("("+ (long)(cluster.get(i).avg)+","+cluster.get(i).count
...
//  }
//
//  public TDigestForExact(InputStream inputStream, int maxMemoryByte, int maxSerializeByte) throws IOException {
//    this(maxMemoryByte);
