...
public class GCHashMapAggressiveForQuantile {
  ...
  public GCHashMapAggressiveForQuantile(int maxSize, int bits, double expectedRoughK,
                                        long[]value, long[]count, IntArrayList index,
                                        long hashMapCount) {
...
//    System.out.println("\t aggressive update: "+ num);
    ...
    if((val^deltaForUnsignedCompare)<(L^deltaForUnsignedCompare))
      countL += freq;
    else if((val^deltaForUnsignedCompare)>(R^deltaForUnsignedCompare))
      countR+=freq;
    else {
      hashMapCount+=freq;
...
//    System.out.println("\t\t\t totCount:"+totCount+"  countL,R,mid:"+countL+" "+countR+" "+hashMapCount);
    if(expectedRoughK*totCount<countL||countL+hashMapCount<expectedRoughK*totCount)
      return false;
    double rate = 0.5;
    int expectPos = 0;long tmpCount = countL;
    for(int i=0;i<length;i++){
      tmpCount+=count[index.getInt(i)];
      if(tmpCount<=expectedRoughK*totCount)
        expectPos = i;
      else break;
    }
...
//    System.out.println("\t\t\t"+value[index.getInt(expectPosL)]+"..."+value[index.getInt(expectPosR)]);
    ...
    hashMapCount = 0;
    for(int i=0;i<expectPosL;i++)
      countL+=count[index.getInt(i)];
    for(int i=expectPosR+1;i<length;i++)
      countR+=count[index.getInt(i)];
    for (int i = expectPosL; i <= expectPosR; i++) {
      x = index.getInt(i);
      v = value[x] + (1L << (bitsOfValue - remainingBits)) / 2.0;
      if (stdVar < 1e-9 || Math.abs(v - expectV) <= 4.0 * stdVar) {
        if (aggressiveHashMap.isEmpty())
          L = value[x];
...
//    System.out.println("\t\t\t aggressive rebuild over. L,R:"+L+"..."+R+"  countL,R,mid:"+countL+" "+countR+" "+hashMapCount);
    return true;
  }
