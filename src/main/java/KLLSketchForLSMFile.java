...
//    while((1<<additionalLevel)<subSketchList.size())additionalLevel++;
    ...
    int subLevel = subSketchList.get(0).cntLevel, targetLevel = subLevel+additionalLevel, tmpNumLen=0;
...
//System.out.println("\t\t[CompactSubSketchesInLSM:] addLV:"+additionalLevel);
    ...
    int cntNumLen=0;
    for(KLLSketchForQuantile subSketch:subSketchList) {
      System.arraycopy(subSketch.num, subSketch.levelPos[subLevel - 1], tmpNum, cntNumLen, subSketch.getLevelSize(subLevel - 1));
