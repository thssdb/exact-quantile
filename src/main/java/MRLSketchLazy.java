...
public class MRLSketchLazy {
  ...
  public MRLSketchLazy(int maxMemoryByte) {
    N=0;minV=Long.MAX_VALUE;maxV=Long.MIN_VALUE;
...
//    System.out.println("T_T\t"+(R1-L1));
  ...
  protected void compact(){
    ...
    for(int i=0;i<cntLevel;i++)if(getLevelSize(i)>maxLevelSize){
      compactOneLevel(i);
      compacted=true;
      break;
    }
...
//    System.out.println("\t\t[]exactKLL lowerBound.\t\t\trank:"+queryRank+"\t\t\t\tlowerBoundV:"+L+"(longToResult:"+longToResult(L)+")"+ "\t\tL_rank:"+getApproxRank(L)+"\t\tErr:"+0);
    return L;
  }
