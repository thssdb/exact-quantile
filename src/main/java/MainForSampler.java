...
//        return seed;
    ...
    private long nextNumber(long x, int i) {
        long num=x;
//        num=x&67108863;
        num = (long)(random.nextGaussian()*1e7);
//        num = (x&127)+128;
        long rnd = Math.abs(nextLong())%10000;
        if(rnd<=2000)
            num=Math.abs(nextLong())%1000000000000L;
        else if(rnd<=4000)
            num=Math.abs(nextLong())%10000000000000000L;
//            num = nextLong();
        return num;
    }
    public void test1(){
...
//        System.out.println("   end mem:"+1.0*(Runtime.getRuntime().totalMemory() - Runtime.getRuntime().freeMemory())/1024/1024);
    ...
    public void test2(){
//        System.out.println("   start mem:"+1.0*(Runtime.getRuntime().totalMemory() - Runtime.getRuntime().freeMemory())/1024/1024);
        SamplerForMedian worker = new SamplerForMedian(131072*3);
        reset();
        Random random = new Random(233);
        long mmp=0;
        for(int i=1;i<=N;i++) {
...
//            }
            ...
            MainForSampler main = new MainForSampler();
            main.test1();
...
//            }
        ...
        System.out.println("sampler: "+sum/TEST_CASE+"ms");
