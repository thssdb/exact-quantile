...
public class KLLSketchForSST extends KLLSketchForQuantile {
  ...
  public void compactSubSketches(int SketchSizeRatio){
    ...
    for(int lv=levelPos[0]=0;lv<cntLevel;lv++){
      ...
      for(KLLSketchForQuantile sketch:subSketchList)if(sketch.cntLevel>lv&&sketch.getLevelSize(lv)>0){
        System.arraycopy(sketch.num,sketch.levelPos[lv],num,levelPos[lv+1],sketch.getLevelSize(lv));
        levelPos[lv+1]+=sketch.getLevelSize(lv);
      }
      if(getLevelSize(lv)>0)
        Arrays.sort(num,levelPos[lv],levelPos[lv+1]);
    }
    level0Sorted=true;
