 ...
 */
public class HppcHashMapForQuantile {
  ...
  public HppcHashMapForQuantile(int bits, int minBits) { // simply 16-bit bucket when remainingBits<=minBits.
    this.minBits = minBits;
    bitsOfValue = remainingBits = bits;
    isBucket = false;

    hashMap = new LongLongHashMap(maxSize+3);
...
//    System.out.println("[rebuild]+remaining:"+remainingBits+"  tot:"+totSize);
    ...
    LongLongHashMap newMap = new LongLongHashMap(maxSize+3);
    for (LongLongCursor c : hashMap)
      newMap.addTo(c.key >>> SHR, c.value);
