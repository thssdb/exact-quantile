...
public class KLLSketchLazyExact extends KLLSketchForQuantile {
  ...
  public KLLSketchLazyExact(int maxMemoryByte) {
    N = 0;
...
//    int newK = 0;
//    for (int addK = 1 << 28; addK > 0; addK >>>= 1) { // find a new K to fit the memory limit.  compact when size>cap. sum of cap<maxMemNum
//      int need = 0;
//      for (int i = 0; i < cntLevel; i++)
//        need += Math.max(3, (int) Math.round(((newK + addK) * Math.pow(2.0 / 3, cntLevel - i - 1))));
//      if (need <= maxMemoryNum) newK += addK;
//    }
//    for (int i = 0; i < cntLevel; i++)
//      levelMaxSize[i] = Math.max(3, (int) Math.round((newK * Math.pow(2.0 / 3, cntLevel - i - 1))));
