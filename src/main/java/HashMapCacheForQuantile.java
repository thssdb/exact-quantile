 ...
 */
public class HashMapCacheForQuantile {
  ...
  public HashMapCacheForQuantile(int bits, int minBits) { // simply 16-bit bucket when remainingBits<=minBits.
    ...
    valueCount = new long[(maxSize + 1)<<1];
...
//    System.out.println("[turnToBucket]+remaining:"+remainingBits+"  tot:"+totSize);
    ...
    for(int i=1;i<=totSize;i++)
      bucket[(int)(valueCount[i<<1]>>> (remainingBits - bucketBits))] += valueCount[i<<1|1];
...
//    System.out.println("[rebuild]+remaining:"+remainingBits+"  tot:"+totSize);
    ...
    deltaForUnsignedCompare = 0;
    for (int i = 1; i <= maxSize; i++)
      insertHashMap(valueCount[i<<1] >>> 1, valueCount[i<<1|1]);
  }

  private void insertHashMap(final long num, final long freq) { // as list
    int index = (int) (hash(num) & 65535), x = root[index];
    if (x != 0) {
//      int last=0;
      while (valueCount[x<<1] != num && rc[x] != 0) {/*last=x;*/x = rc[x];/*DEBUG++;*/}
      if (valueCount[x<<1] == num) {
        valueCount[x<<1|1] += freq;
      } else {
        rc[x] = ++totSize;
        valueCount[totSize<<1] = num;
        valueCount[totSize<<1|1] = freq;
      }
...
//      }
  ...
  public List<Long> findResultIndex(long K1, long K2) {
    ...
    if(isBucket){
      ...
      index.sort((x, y) -> Long.compare(valueCount[x<<1]^deltaForUnsignedCompare, valueCount[y<<1]^deltaForUnsignedCompare));
      int x;
      for (int i = 0; i < totSize; i++) {
        x = index.getInt(i);
//      System.out.println(count[x] + "  " + value[x]);
        sum += valueCount[x<<1|1];
        if (sum >= K1 && result.size() == 0) {
          result.add(valueCount[x<<1]);
          result.add(sum - valueCount[x<<1|1]);
        }
        if (sum >= K2 && result.size() == 2) {
          result.add(valueCount[x<<1]);
          result.add(sum - valueCount[x<<1|1]);
