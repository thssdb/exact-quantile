...
public class TDigestCachedForMedian{
  ...
  public TDigestCachedForMedian (int compression,int bufferSizeLimit) {
    ...
    radixCount = new long[1<<16];
    radixNum = new long[bufferSizeLimit];
  }
  private void radix(long[] a, long[] b, final int SHR){
    for(int i=0;i<bufferNum;i++)
      radixCount[(int)(a[i]>>>SHR&65535)]++;
    for(int i=1;i<65536;i++)
      radixCount[i]+=radixCount[i-1];
    for(int i=bufferNum-1;i>=0;i--)
      b[(int)(--radixCount[(int)(a[i]>>>SHR&65535)])]=a[i];
  }

  private void sortBuffer(){
//    Arrays.sort(buffer, 0, bufferNum);
    radix(buffer, radixNum, 0);
    radix(radixNum, buffer, 16);
    radix(buffer, radixNum, 0);
    radix(radixNum, buffer, 16);
  }
