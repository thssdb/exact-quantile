...
public class KLLSketchLazyForVerifyOddEvenIntervalRate {
  ObjectArrayList<IntObjectPair<LongArrayList>> compactRecord = new ObjectArrayList<>();
  long N;
  int maxMemoryNum;
  long[] num;
  ...
  static double[][] PR;
  static final int maxTimesForExact = 4,maxSearchNum=400,iterLVNum=/*1*/0;
  ...
  public KLLSketchLazyForVerifyOddEvenIntervalRate(int maxMemoryByte) {
    N = 0;
  ...
  public static int[] calcLevelMaxSizeByLevel(int maxMemoryNum,int setLevel) {
    ...
    for (int addK = 1 << 28; addK > 0; addK >>>= 1) { // find a new K to fit the memory limit.
      ...
      for (int i = 0; i < setLevel; i++)
        need += Math.max(3, (int) Math.round(((newK + addK) * Math.pow(2.0 / 3, setLevel - i - 1))));
...
//    System.out.println("T_T\t"+(R1-L1));
    ...
    LongArrayList elementInCompaction = new LongArrayList();
    for(int i=L1;i<R1;i++)elementInCompaction.add(num[i]);
//    System.out.println("\t\t\tcompact\tmnRk,mxRk:\t"+num[L1]+","+num[R1-1]+"\t\tcompactLevelSize:\t"+(R1-L1));
    compactRecord.add(IntObjectPair.of(level,elementInCompaction));
  ...
  public void compact() {
    ...
    for (int i = 0; i < cntLevel; i++)
      if (levelPos[i + 1] - levelPos[i] > levelMaxSize[i]){
        if(mxLV==-1||getLevelSize(i)>getLevelSize(mxLV))mxLV=i;
      }
    if(mxLV!=-1){compactOneLevel(mxLV);return;}

//    int wmxLV=-1;
//    for (int i = 0; i < cntLevel; i++)
//      if (levelPos[i + 1] - levelPos[i] > levelMaxSize[i]){
//        if(wmxLV==-1||(getLevelSize(i)>>>(i*2))>(getLevelSize(wmxLV)>>>(wmxLV*2)))wmxLV=i;
//      }
//    if(wmxLV!=-1){compactOneLevel(wmxLV);return;}
...
//    System.out.println();
  ...
  private void addRecordInLevel(long minV, long maxV, int level) {
    ...
    long hisMinV = compactionMinVInLevel.getLong(level),hisMaxV=compactionMaxVInLevel.getLong(level);
    if(maxV<=hisMinV||minV>=hisMaxV){
      hisNum = Math.max(hisNum,1);
//      if(level>=9)
//      System.out.println("\t\t\t\trecord extreme.\t"+longToResult(minV)+"\t"+longToResult(maxV)+"\t\t\tnewnum:"+hisNum+"\t\tLV:"+level);
    }
        else
        {
      hisNum++;
//      if(level>=9)
//        System.out.println("\t\t\t\tnormal record.\t"+longToResult(minV)+"\t"+longToResult(maxV)+"\t\t\tnewnum:"+hisNum+"\t\tLV:"+level);
    }
  ...
  public double getSumMWeight2(){ // sum{ compactNum * weight^2 }
    double sum=0;
    for(int i=0;i<cntLevel-1;i++)sum+=compactionNumInLevel.getInt(i)*Math.pow((1<<i),2);
    return sum;
  }


  // return bound of result with possibility >= p
  private static double prOutOfBoundDFS(double cntPR, int level, int topErr, int bound, int lowLevel, NormalDistribution lowDis, int lowExtreme, int[] relatedCompactNum) {
...
//      System.out.println("\t\t\t\t\t\t\t\t\t\tcntErr:"+cntErr+"\t\tlevel="+level+"\t\tx:"+x+"\t\tcntLevelPr="+PR[cntTime][x+cntTime]+"\t\t\tcntLevelCompNum:"+cntTime+"\tcntLevelV:"+x);
  ...
  private static boolean checkBound(int bound, double Pr, int lowLevel, NormalDistribution lowDis, int lowExtreme, int[] relatedCompactNum) {
    double prOutOfBound = prOutOfBoundDFS(1.0, relatedCompactNum.length-1, 0, bound, lowLevel, lowDis, lowExtreme, relatedCompactNum);
...
//    }
    ...
    for (int i = 1; i < compLevel; i++) maxERRBottom[i] = maxERRBottom[i - 1] + (relatedCompactNum[i] << i);
    for (int i = 0; i < compLevel; i++) {
      double all_case = 1.0;
      for (int j = i; j < compLevel; j++) all_case *= relatedCompactNum[j] * 2 + 1;
      if (relatedCompactNum[i] >= maxTimesForExact || all_case > maxSearchNum || compLevel-i>iterLVNum) {
        sig2 += relatedCompactNum[i] / 2.0*Math.pow(2,i*2);
        lowLevel = i + 1;
        lowExtreme += relatedCompactNum[i] << i;
      }
    }
    NormalDistribution lowDis = sig2 > 0 ? (sig2==lastSig2?lastNormalDis:new NormalDistribution(0, Math.sqrt(sig2))) : null;
    lastSig2=sig2;
...
//      if (relatedCompactNum[i] >= maxTimesForExact || all_case > maxSearchNum || cntLevel-1-i>iterLVNum) {
//        sig2 += relatedCompactNum[i] / 2.0*Math.pow(2,i*2);
//        lowLevel = i + 1;
//        lowExtreme += relatedCompactNum[i] << i;
//      }
